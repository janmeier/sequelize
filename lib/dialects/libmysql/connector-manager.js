...
module.exports = (function() {
      ...
      this.pool = {
        ...
        read: Pooling.Pool({
          ...
          create: function (done) {
            ...
            connection.connectSync(config.host, config.username, config.password, config.database, config.port);
  ...
  var connect = function(done) {
    var connection = mysql.createConnectionSync();
    //connection.setOptionSync(connection.MYSQL_SET_CHARSET_NAME, "utf8");
    
    connection.connectSync(this.config.host, this.config.username, this.config.password, this.config.database, this.config.port);
    connection.setCharsetSync('utf8');
