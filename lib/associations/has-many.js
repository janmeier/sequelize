...
module.exports = (function() {
    ...
    if ((this.isSelfAssociation && this.useJunctionTable) || multiAssociation) {
      ...
      if(Utils.isHash(this.identifier)) {
        Utils._.each(this.identifier, function(elem, key) {
          combinedTableAttributes[key] = self.identifier[key].type || DataTypes.INTEGER
        })
      } else {
        combinedTableAttributes[this.identifier] = { type: DataTypes.INTEGER }
      }
      if(Utils.isHash(this.foreignIdentifier)) {
        Utils._.each(this.foreignIdentifier, function(elem, key) {
          combinedTableAttributes[key] = self.foreignIdentifier[key].type || DataTypes.INTEGER
        })
      } else {
        combinedTableAttributes[this.foreignIdentifier] = { type: DataTypes.INTEGER }
      }
  ...
  HasMany.prototype.injectGetter = function(obj) {
    ...
    if (!self.connectorDAO) {
      var where = {};
      where[this.identifier] = obj.id;
      obj[this.accessors.get].findAll = {where: where};
    }
